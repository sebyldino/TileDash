...
window.addEventListener('load', function () {
  ...
  function displayDevices(devices, dashboard) {
    ...
    const groupMargin = window.dashboard.settings.groupMargin;
    ...
    dashboard.forEach(function (group, index) {
      const groupWidth = tileWidth * group.width + (group.width + 1) * tileMargin;
      const groupHeight = tileHeight * group.height + (group.height + 1) * tileMargin;
      ...
      group.items.forEach(function (item) {
        ...
        if ((device || type === 'IMAGE' || type === 'VIRTUAL' || type === 'VIRTUAL_POPUP') && type != 'DOORBIRD_POPUP') {
          ...
          if (item.capabilityID) $device.id = 'device-' + item.id + '-' + item.capabilityID;
          else $device.id = 'device-' + item.id;
          ...
          if (type === 'SWITCH') createSwitch(device, item, $device, smallDevice, devices);
          else if (type === 'BUTTON') createButton(device, item, $device, smallDevice, devices);
          else if (type === 'IMAGE') createImage(device, item, $device, smallDevice, devices);
          else if (type === 'SENSOR') createSensor(device, item, $device, smallDevice, devices);
          else if (type === 'BINARY_SENSOR') createBinarySensor(device, item, $device, smallDevice, devices);
          else if (type === 'SLIDER') createSlider(device, item, $device, smallDevice, devices);
          else if (type === 'THERMOSTAT') createThermostat(device, item, $device, smallDevice, devices);
          else if (type === 'POPUP' || type === 'VIRTUAL_POPUP') createPopup(device, item, $device, smallDevice, devices);
          else if (type === 'SHUTTER') createShutter(device, item, $device, smallDevice, devices);
          else if (type === 'HEIMDALL') createHeimdall(device, item, $device, smallDevice, devices);
          else if (type === 'VIRTUAL') createVirtual(device, item, $device, smallDevice, devices);
          else if (type === 'MEDIA') createMedia(device, item, $device, smallDevice, devices);
          else {
        ...
        if (type === 'DOORBIRD_POPUP') createDoorbirdPopup(device, item);
