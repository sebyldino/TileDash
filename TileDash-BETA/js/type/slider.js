////=====================    Type SLIDER   ======================
function createSlider(device, item, $device, smallDevice, devices, ratio, extraID, smooth) {
  var multiplier = 1;
  ...
  if (item.icon) {
    ...
    const value = device.capabilitiesObj[item.capabilityID].value || '0';
    if (parseFloat(value) > 0 && smooth) {
      $icon.classList.add('smooth-icon-on');
    }
  ...
  if (item.capabilityID) {
    ...
    if (parseFloat($slider.value) > 0 && !smooth) {
      ...
      if ($device) {
        ...
        if (parseFloat(value) > 0) {
          if (!smooth) $device.classList.add('on');
            ...
            const icon = document.querySelector(`#${$device.id} i`);
            icon.classList.remove('smooth-icon-off');
            icon.classList.add('smooth-icon-on');
          }
        } else {
          if (!smooth) $device.classList.remove('on');
            ...
            const icon = document.querySelector(`#${$device.id} i`);
            icon.classList.remove('smooth-icon-on');
            icon.classList.add('smooth-icon-off');
          }
        }
      }
    });
